 ...
 * @param {string} hmacAlgorithm Hash algorithm used for computing the HMAC of the given seed pair.
...
export function randomInt(
  ...
  // Validate parameters
  if (size <= 0 || size > 6) {
    throw new RangeError('Size must be non-negative and less than or equal to 6.');
  }

  if (range <= 0 || range > maxRange) {
    throw new RangeError('Max - min must be non-negative and less than 256 ** size.');
  }
